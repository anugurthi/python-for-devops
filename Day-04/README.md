...
Today’s goal is to evolve your scripts into reusable libraries. You’ll keep this day’s notes as a reference whenever you split code into helpers across the remaining automation projects.

## Learning Goals

- Wrap repeated logic in functions with clear inputs/outputs.
- Organize helpers into modules and import them cleanly.
- Understand how packages enable multi-file Python tooling.

## Agenda

1. Read through the primer below on functions, modules, and packages.
2. Extract repeated string utilities from earlier days into `utils/string_tools.py`, then compare against `examples/function_refactor.py` for a reference implementation.
3. Create a `__init__.py` that exposes only the functions you want other scripts to use.
4. Document how to run module-specific unit tests.
